...
export GCOV_ROOT="/confetti-gcov"
export KLEE_ROOT="/confetti-klee"
export SYMCC_ROOT="/confetti-symcc"
export SYMSAN_ROOT="/confetti-symsan"
export AFLPLUSPLUS_ROOT="/confetti-aflplusplus"
export INSTR_ROOT="/confetti-instr"
export BIN="parse"

export TESTER_EXECUTE_ARGS=""    # no arguments and stdin
export GCOVE_EXECUTE_ARGS="<"
export KLEE_SYM_ARGS="--sym-stdin 242 "
...
setup_concolic_environment() {
    ...
    # NOTE: upstream run.sh does `git fetch && git reset --hard origin/main` here.
    ...
    echo "git version: $git_version (local working tree, not reset)" >> ${SHARED_DIR}/execution_command.log
...
case $TESTER_NAME in
  ...
  "klee" | "klee-pending")
    ...
    timeout -k 10 $TIMEOUT_SECONDS /bin/bash -c \
        "klee --max-memory=8000 --seed-file=${INPUT}/in.ktest --only-output-states-covering-new --simplify-sym-indices --disable-inlining --switch-type=simple --external-calls=all \
...
# collect coverage
if [ "$TESTER_NAME" != "concolic" ]; then
    echo "Time,l_per,l_abs,b_per,b_abs,covered_times_of_lines" > $covfile
